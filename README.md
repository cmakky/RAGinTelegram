...
- **`/list`** -- какие файлы загружены и сколько чанков у каждого;
- **`/delete имя_файла`** -- удалить конкретный файл из индекса;
- **`/reset confirm`** -- полностью очистить все данные пользователя.
...
│       ├── vector_store.py         # обертка над ChromaDB (персистентная векторная база)
